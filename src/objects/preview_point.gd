...
enum State { HIDDEN, VALID_NEW_POINT, INVALID_NEW_POINT, SNAP_TO_POINT, SNAP_TO_ENEMY_POINT }
...
func update_state(player):
	...
	for point in all_points:
		...
		for point2 in point.get_connection_points():
			...
			var intersects = Utils.are_segments_intersecting(
				player.position, player.position + position, point.position, point2.position
			)
			if intersects:
	...
	for point in all_points:
		...
		var distance = Utils.distance_point_to_segment(
			point.position, player.position, player.position + position
		)
